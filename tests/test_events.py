...
from app.models import Story, Character, DiscoveryQuestion, DiscoveryAnswer, DiscoveryEvent
...
def test_discovery_events_and_feed(client: TestClient, db: Session):
    ...
    res = client.post("/api/v1/discovery/answers", json={
        "story_id": story_id,
    ...
    events = db.query(DiscoveryEvent).filter(DiscoveryEvent.story_id == uuid.UUID(story_id)).order_by(DiscoveryEvent.created_at).all()
