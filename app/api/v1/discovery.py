...
from app.models import DiscoveryQuestion, DiscoveryAnswer, FlowTypeEnum, Story, Character, Relationship
...
def get_discovery_questions(flow_type: FlowTypeEnum = Query(...), db: Session = Depends(get_db)):
    questions = db.query(DiscoveryQuestion).filter(DiscoveryQuestion.flow_type == flow_type).order_by(DiscoveryQuestion.order_index).all()
    return questions
...
def create_discovery_answer(answer_in: DiscoveryAnswerCreate, db: Session = Depends(get_db)):
    ...
    if question.flow_type == FlowTypeEnum.CHARACTER_DISCOVERY:
        ...
        if not db.query(Character).filter(Character.id == answer_in.character_id, Character.story_id == answer_in.story_id).first():
        ...
        if not answer_in.relationship_id:
            raise HTTPException(status_code=400, detail="relationship_id is required for relationship discovery questions")
        if not db.query(Relationship).filter(Relationship.id == answer_in.relationship_id, Relationship.story_id == answer_in.story_id).first():
    ...
    if answer_in.character_id:
        existing_answer = db.query(DiscoveryAnswer).filter(
