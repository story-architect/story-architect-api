...
def secure_file(filepath):
    ...
    if "get_current_user" not in content:
        content = content.replace(
            "from app.api.dependencies import get_db", "from app.api.dependencies import get_db, get_current_user"
        )
    ...
    def replacer(match):
        ...
        if "db.query(Story).filter(Story.id == story_id)" in body:
            body = body.replace(
                "db.query(Story).filter(Story.id == story_id)",
                "db.query(Story).filter(Story.id == story_id, Story.user_id == current_user.id)",
            )
        elif "db.query(Relationship).filter(Relationship.id == relationship_id)" in body:
            body = body.replace(
                "db.query(Relationship).filter(Relationship.id == relationship_id)",
                "db.query(Relationship).join(Story).filter(Relationship.id == relationship_id, Story.user_id == current_user.id)",
            )
        elif "db.query(DiscoveryEvent).filter(DiscoveryEvent.id == event_id)" in body:
            body = body.replace(
                "db.query(DiscoveryEvent).filter(DiscoveryEvent.id == event_id)",
                "db.query(DiscoveryEvent).join(Story).filter(DiscoveryEvent.id == event_id, Story.user_id == current_user.id)",
            )
